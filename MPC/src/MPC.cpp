...
// calculate start index of each state, because it will be saved in a 1D vector
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // set all non-actuators upper and lower limits to the max positive and negative values
  for (int i = 0; i < delta_start; i++) {
    vars_lowerbound[i] = -FLT_MAX; // FLT_MAX is the maximum value of float
    vars_upperbound[i] = FLT_MAX;
  }

  // the upper and the lower limits of delta are set to -25 and 25 degrees (values in radians)
